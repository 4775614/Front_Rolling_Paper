...
//댓글 작성 Post - ("/comments")
export const postingComment = createAsyncThunk(
	'postingComment',
	async (payload, thunkAPI) => {
		try {
			const response = await instance.post(
				`/comments`,
				{
					rollingPaperId: payload.rollingPaperId,
...
export const getComment = createAsyncThunk(
	...
	async (payload, thunkAPI) => {
		try {
			const response = await instance.get(`/comments/${payload}`);
			if (response.data.success === true) {
				return thunkAPI.fulfillWithValue(response.data.result);
			}
		} catch (error) {
...
export const paperSlice = createSlice({
	...
	extraReducers: {
		...
		[postingComment.fulfilled]: (state, action) => {
